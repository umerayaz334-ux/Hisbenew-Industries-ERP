...
def build_tspl_job(
    ...
    for item in labels:
        ...
        # Native TSPL Text & Barcode Centering (Exact Vector Scaling)
        title_char_w = 16 * (dots_per_mm / 11.81)
        title_w = len(title[:35]) * title_char_w
        x_title = max(15, round((width_dots - title_w) / 2))

        barcode_w = (len(barcode_val[:25]) + 3) * 22 * (dots_per_mm / 11.81)
        x_barcode = max(15, round((width_dots - barcode_w) / 2))

        sku_char_w = 24 * (dots_per_mm / 11.81)
        sku_w = len(sku[:20]) * sku_char_w
        x_sku = max(15, round((width_dots - sku_w) / 2))
        ...
        # Code 128 Barcode in middle (Centered, no human readable below bar)
        if barcode_val:
